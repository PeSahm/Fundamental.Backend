...
3. **Create Canonical Entity** (`Entities/Canonical[StatementName].cs`)
   ...
   **CRITICAL STRUCTURE PATTERN** (Follow CanonicalInterpretativeReportSummaryPage5):
   
   **Metadata Properties at Root Level (with private setters)**:
     - `Version` (string - "V1", "V2", etc.) - private setter
     - `PublishDate` (DateTime, UTC) - private setter
     - `Symbol` (navigation property) - private setter
     - `TraceNo` (ulong) - private setter
     - `HtmlUrl` or `Uri` (Uri) - private setter
     - `FiscalYear` (FiscalYear value object) - private setter
     - `Currency` (IsoCurrency enum) - private setter
     - `YearEndMonth` (StatementMonth value object) - private setter
     - `ReportMonth` (StatementMonth value object) - private setter
     - Additional domain-specific dates (e.g., AssemblyDate) - private setter
   
   **Owned Entities for Nested JSON Structures** (preserve JSON hierarchy):
     - If JSON has nested objects (e.g., `parentAssembly`, `assemblyChiefMembers`), create owned entity types
     - Use **public setters** for owned entities (required for mapping)
     - Example: `public ParentAssemblyInfo? ParentAssemblyInfo { get; set; }`
   
   **Collections at Root Level** (public setters):
     - Only collections that appear at JSON root level
     - Example: `public ICollection<ShareHolder> ShareHolders { get; set; } = new List<ShareHolder>();`
   
   **Nullable Owned Entities** (public setters):
     - For optional single objects (e.g., CEO, AuditCommittee)
     - Example: `public AssemblyAttendee? Ceo { get; set; }`
   
   - **Constructor**: Include ALL required metadata properties
     ```csharp
     public Canonical[StatementName](
         Guid id,
         Symbol symbol,
         ulong traceNo,
         Uri htmlUrl,
     ...
     ```
   - Initialize collections in constructor body
...
1. **Create Configuration** (`Canonical[StatementName]Configuration.cs`)
   ...
   **CRITICAL**: Use `EntityTypeConfigurationBase<T>` pattern, NOT `IEntityTypeConfiguration<T>`
   
   ```csharp
   public class Canonical[StatementName]Configuration 
       : EntityTypeConfigurationBase<Canonical[StatementName]>
   {
       protected override void ExtraConfigure(EntityTypeBuilder<Canonical[StatementName]> builder)
       {
           // Table with snake_case naming
           builder.ToTable(
               NpgsqlSnakeCaseNameTranslator.ConvertToSnakeCase(nameof(Canonical[StatementName])),
               "manufacturing");
           
           // Shadow FK (handled by base class, but configure navigation)
           ...
           builder.Property(x => x.TraceNo).IsRequired();
           builder.Property(x => x.HtmlUrl).IsRequired();
           ...
           // Currency
           builder.Property(x => x.Currency).Currency();
           
           // Owned entities (if JSON has nested structures)
           builder.OwnsOne(
               x => x.ParentInfo,
               navigationBuilder =>
               {
                   navigationBuilder.ToJson("parent_info");
                   // Configure nested collections inside owned entity
                   navigationBuilder.OwnsMany(x => x.NestedItems);
               });
           ...
           builder.OwnsMany(
               ...
               navigationBuilder =>
               {
                   navigationBuilder.ToJson("section1_items");
                   // Enum conversions happen automatically with EF Core
               });
           // Repeat for all collections
           
           // Nullable owned entities
           builder.OwnsOne(
               x => x.Ceo,
               navigationBuilder =>
               {
...
2. **Private Setters**: 
   - Metadata properties at root: **private setters**
   - Owned entities: **public setters**
   - Collections: **public setters**
   - Exception: Properties that need to be updated by mapping service
...
10. **Configuration Pattern**: Use `EntityTypeConfigurationBase<T>` with `ExtraConfigure` method
11. **JSON Structure**: Preserve original JSON nesting with owned entities, don't flatten unnecessarily
12. **Constructor Parameters**: Include ReportMonth in addition to YearEndMonth
...
- [ ] Canonical entity structure follows pattern:
  - [ ] Metadata properties at root with private setters
  - [ ] Owned entities for nested JSON with public setters
  - [ ] Collections at root with public setters
  - [ ] Constructor includes all required parameters (including ReportMonth)
...
- [ ] EF Core configuration:
  - [ ] Uses `EntityTypeConfigurationBase<T>` pattern
  - [ ] Uses `ComplexProperty` for FiscalYear
  - [ ] Uses `OwnsOne` for YearEndMonth and ReportMonth
  - [ ] Uses `OwnsOne().ToJson()` for nested owned entities
  - [ ] Uses `OwnsMany().ToJson()` for collections
  - [ ] Properly configures nested collections inside owned entities
- [ ] All services registered in DI container
- [ ] 8+ integration tests created and passing
- [ ] Test data JSON file copied to output directory
- [ ] Build succeeds with zero errors
- [ ] No magic numbers in code
- [ ] JSON structure preserved (not unnecessarily flattened)

### 🚀 Example Reference Implementation

**Study these files for complete pattern:**
- `CanonicalInterpretativeReportSummaryPage5.cs` - **BEST EXAMPLE** for entity structure with owned entities
- `CanonicalAnnualAssembly.cs` - Shows nested owned entities (ParentAssemblyInfo, AssemblyChiefMembersInfo)
- `CanonicalMonthlyActivity.cs` - Domain entity structure (simpler, flat structure)
- `AnnualAssemblyMappingServiceV1.cs` - Mapping pattern with owned entities
- `MonthlyActivityV5Processor.cs` - Processor pattern
- `AnnualAssemblyIntegrationTests.cs` - Testing pattern with owned entities
- `CanonicalAnnualAssemblyConfiguration.cs` - **BEST EXAMPLE** for EF Core configuration with EntityTypeConfigurationBase
- `CanonicalInterpretativeReportSummaryPage5Configuration.cs` - Alternative EF Core pattern
...
|---------|---------|----------|
| Using `IEntityTypeConfiguration<T>` | Wrong pattern for this project | Use `EntityTypeConfigurationBase<T>` with `ExtraConfigure` method |
...
| Flattening nested JSON to root properties | Loses original JSON structure | Create owned entities to preserve nesting (e.g., ParentAssemblyInfo) |
| Missing `ReportMonth` in constructor | Incomplete metadata | Always include: FiscalYear, YearEndMonth, ReportMonth |
| Using `.HasColumnName()` for value objects | Verbose and error-prone | Use `ComplexProperty` or `OwnsOne` pattern |
| Not handling null checks in Specifications | Runtime errors | Use null-conditional operators: `x.ParentInfo != null ? x.ParentInfo.Value : null` |
| Accessing nested collections without null checks | Runtime errors | Check owned entity exists before accessing its collections |
| Using public setters for metadata properties | Breaks immutability | Use private setters for metadata, public for owned entities/collections |
| Setting `yearEndMonth` from assembly date month | Wrong value | Default to 12 (end of Persian year) or extract from specific field |
